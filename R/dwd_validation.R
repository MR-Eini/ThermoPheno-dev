...
#'
#' Performs structural checks for weather tables used by ThermoPheno and adds
#' practical quality checks inspired by typical Deutscher Wetterdienst daily
#' temperature datasets.
#'
#' @param df A data frame containing at least `date`, `tmin`, and `tmax`.
#' @param strict If `TRUE`, stop on validation errors. If `FALSE`, return issues.
  ...
  if (!is.data.frame(df)) {
    ...
    if (length(errors) == 0) {
      ...
      if (any(is.na(parsed_dates))) {
        errors <- c(errors, "Column `date` contains unparseable values (expected YYYY-MM-DD or Date-compatible values).")
      }
      ...
      if (length(parsed_dates) > 1 && all(!is.na(parsed_dates))) {
        if (any(duplicated(parsed_dates))) {
